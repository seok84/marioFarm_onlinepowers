...
*/
	...
	// 상품 구매하기 - mobile
	$(document).on('click', '.view_info .btn_area1 .btn_buy, .view_info .btn_open, .viewBg', function() {
		if ($('.view_info .option_area').hasClass('on')) {
			$('.view_info .option_area').removeClass('on');

			$('html, body').removeClass('scroll_none');
			$('.viewBg').hide();
		} else {
			$('.view_info .option_area').addClass('on');
	...
	// 상품옵션 - mobile
	$('.detail_view_wrap').on('click','.dropdown_box .custom-select',function(){
		// $('.type_option').addClass('on');
		$('.detail_view_wrap .option_drop').hide();

		if(!$(this).next('.option_drop').is(':visible')){
			$(this).next('.option_drop').show();
		}
	});
	$('.detail_view_wrap').on('click','.drop_close',function(){
		// $('.view_info .type_option').removeClass('on');
		$('.detail_view_wrap .option_drop').hide();
	});
...
*/
	...
	//따라 다니는 바로구매
	$('.option_area').on('click','.dropdown_box .custom-select',function(){
		// $('.option_area .type_option').addClass('on');
		$('.option_area .option_drop').hide();

		if(!$(this).next('.option_drop').is(':visible')){
			$(this).next('.option_drop').show();
		}
	});
	$('.option_area').on('click','.drop_close',function(){
		// $('.option_area .view_info .type_option').removeClass('on');
		$('.option_area .option_drop').hide();
	});
